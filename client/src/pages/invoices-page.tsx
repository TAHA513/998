...
export default function InvoicesPage() {
  ...
  const addProduct = (product: Product) => {
    setItems(prevItems => {
      ...
      return [...prevItems, { productId: product.id, name: product.name, quantity: 1, price: product.sellingPrice, total: product.sellingPrice }];
    });
  };
      ...
      <html dir="rtl">
        <head>
          ...
          <style>
            ...
            .logo { max-width: 60mm; max-height: 60mm; margin: 0 auto 10px; }
            .store-name { font-size: 16px; font-weight: bold; margin-bottom: 5px; }
            .invoice-details { margin-bottom: 10px; padding: 5px 0; border-bottom: 1px dashed #000; }
            .items { width: 100%; border-collapse: collapse; margin: 10px 0; }
            .items th { border-bottom: 1px solid #000; padding: 5px; text-align: right; font-size: 11px; }
            .items td { padding: 5px; text-align: right; font-size: 11px; }
            .totals { margin-top: 10px; padding-top: 10px; border-top: 1px dashed #000; }
            .footer { margin-top: 20px; text-align: center; font-size: 10px; padding-top: 10px; border-top: 1px dashed #000; }
            .divider { border-bottom: 1px dashed #000; margin: 10px 0; }
          </style>
        </head>
        <body>
          <div class="header">
            ${storeSettings?.storeLogo ? `<img src="${storeSettings.storeLogo}" class="logo" alt="شعار المتجر" />` : ''}
            ${storeSettings?.storeName ? `<div class="store-name">${storeSettings.storeName}</div>` : ''}
          </div>
          ...
          <div class="totals">
            ...
            ${discount > 0 ? `<div>الخصم (${discount}%): ${formatCurrency(discountAmount, true)}</div>` : ''}
            <div style="font-weight: bold; margin-top: 5px;">الإجمالي النهائي: ${formatCurrency(finalTotal, true)}</div>
          </div>
          ${note ? `<div class="divider"></div><div>ملاحظات: ${note}</div>` : ''}
          <div class="footer">
            ${storeSettings?.storeName ? `<div>شكراً لتسوقكم من ${storeSettings.storeName}</div>` : ''}
          </div>
          <script>
  ...
  const saveInvoice = async () => {
    try {
      const invoiceData = {
        customerName,
        items: items.map(item => ({ productId: item.productId, quantity: item.quantity, price: item.price, total: item.total })),
        subtotal,
        discount,
        discountAmount,
        finalTotal,
      ...
      toast({ title: "تم حفظ الفاتورة", description: "تم حفظ الفاتورة بنجاح في النظام" });
    } catch (error) {
      toast({ title: "خطأ في حفظ الفاتورة", description: "حدث خطأ أثناء محاولة حفظ الفاتورة. يرجى المحاولة مرة أخرى.", variant: "destructive" });
    }
  };

  const exportToExcel = () => {
    if (!items || items.length === 0) {
      toast({ title: "لا يمكن تصدير الفاتورة", description: "الفاتورة لا تحتوي على أي منتجات", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      excelData.push(
        { 'اسم المنتج': 'المجموع الفرعي', 'المجموع': `${formatCurrency(subtotal, true)}` }
      );

      if (discount > 0) {
        excelData.push({ 'اسم المنتج': `الخصم (${discount}%)`, 'المجموع': `${formatCurrency(discountAmount, true)}` });
      }

      excelData.push({ 'اسم المنتج': 'الإجمالي النهائي', 'المجموع': `${formatCurrency(finalTotal, true)}` });
      ...
      toast({ title: "تم تصدير الفاتورة", description: "تم تصدير الفاتورة بنجاح إلى ملف Excel" });
    } catch (error) {
      console.error('Error exporting to Excel:', error);
      toast({ title: "خطأ في التصدير", description: "حدث خطأ أثناء تصدير الفاتورة. الرجاء المحاولة مرة أخرى.", variant: "destructive" });
    }
  };
        ...
        <div className="grid gap-4 flex-1 md:grid-cols-[1fr,400px]">
            ...
            <CardContent className="p-4 space-y-4">
              <input
                ...
                onChange={(e) => {
                  ...
                  if (e.target.value.length >= 8) handleBarcodeScan(e.target.value);
                }}
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && barcodeInput) handleBarcodeScan(barcodeInput);
                }}
              />
              <div className="relative">
                <Search className="absolute right-3 top-2.5 h-4 w-4 text-muted-foreground" />
                <Input placeholder="ابحث عن منتج..." className="pr-9" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                  ...
                  <div className="absolute w-full mt-1 p-2 bg-card border rounded-md shadow-lg z-10 max-h-48 overflow-auto">
                    {filteredProducts.map(product => (
                      <button
                        ...
                        onClick={() => { addProduct(product); setSearchQuery(""); }}
                      >
          ...
          <Card>
            <CardContent className="p-4 space-y-4">
              ...
              <div className="space-y-2">
                <Label>ملاحظات</Label>
                <Input value={note} onChange={(e) => setNote(e.target.value)} placeholder="أضف أي ملاحظات إضافية هنا" />
              </div>
